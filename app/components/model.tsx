...
export default function Model({activeMenu}: ModelProps): React.JSX.Element {

    const plane = useRef<THREE.Mesh<THREE.BufferGeometry, THREE.ShaderMaterial>>(null);
    const { viewport } = useThree();
    const dimension = useDimension();
    const mouse = useMouse();
    const opacity = useMotionValue(0);
    const textureUrls = useMemo(() => projects.map(project => project.src), []);
    const textures = useTexture(textureUrls);
    const { width, height } = textures[0].image as HTMLImageElement;
    ...
    return (
        <motion.mesh
            ...
            position-y={y}
            ref={(mesh) => {
                plane.current = mesh as unknown as THREE.Mesh<THREE.BufferGeometry, THREE.ShaderMaterial> | null
            }}
            scale={scale}
        >
